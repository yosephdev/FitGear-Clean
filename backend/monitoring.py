...
class PerformanceMonitor:
    ...
    def log_request(self, endpoint: str, method: str, duration: float, status_code: int):
        ...
        self.metrics['average_response_time'] = ((current_avg * (count - 1)) + duration) / count
...
class SecurityMonitor:
    ...
    def log_failed_login(self, ip_address: str, email: str):
        ...
        if len(self.failed_attempts[key]) >= 5:
            ...
            logger.warning(f"Suspicious activity detected from IP {ip_address}: {len(self.failed_attempts[key])} failed login attempts")
        
        # Block IP after 10 failed attempts
        if len(self.failed_attempts[key]) >= 10:
            self.blocked_ips.add(ip_address)
            logger.critical(f"IP {ip_address} has been blocked due to excessive failed login attempts")
...
class DatabaseMonitor:
    ...
    def log_query(self, operation: str, duration: float):
        ...
        self.connection_stats['average_query_time'] = ((current_avg * (count - 1)) + duration) / count
        
        # Log slow queries
        if duration > 1.0:  # Log queries taking more than 1 second
            logger.warning(f"Slow database query detected: {operation} took {duration:.2f}s")
    
    def get_stats(self) -> Dict[str, Any]:
        """Get database statistics"""
        total_connections = self.connection_stats['successful_connections'] + self.connection_stats['failed_connections']
        success_rate = (self.connection_stats['successful_connections'] / max(total_connections, 1)) * 100
...
class ErrorTracker:
    ...
    def log_error(self, error_type: str, error_message: str, severity: str = "error"):
        ...
        if self.error_counts[error_type] > 10:
            logger.warning(f"Frequent error detected: {error_type} has occurred {self.error_counts[error_type]} times")
